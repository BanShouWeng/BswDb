...
import com.bsw.dblibrary.SPUtils;
...
public class DbBase {
        ...
        for (Field field : fields) {
            ...
            if (primaryKeyAnno) {                                   // 主键
                ...
                try {
                    ...
                    primaryKeyPojo = new ColumnPojo(field, name, null == t ? null : field.get(t), type, true);
                    break;
    ...
    private <T> void reflect(Class clz, T t) {
        ...
        for (Field field : fields) {
            ...
            if (requireAnno) {                                      // 必填项
                ...
                try {
                    ColumnPojo columnPojo = new ColumnPojo(field, name, null == t ? null : field.get(t), type, true);
     ...
     * @param sql       创建表的SQL
     */
    void saveDB(Context mContext, String tableName, String sql) {
        if (null == spUtils) {
     ...
     */
    Map<String, String[]> getDB(Context mContext) {
        if (null == spUtils) {
            spUtils = SPUtils.getInstance(mContext);
        }
        Gson gson = new Gson();
